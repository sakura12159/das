...
from PyQt5.QtWidgets import QApplication, QMainWindow, QAction, QFileDialog, qApp, QMenu, QFormLayout, \
    QTabWidget, QTableWidget, QAbstractItemView, QTableWidgetItem, QHeaderView, QTabBar, QWidget, QScrollArea
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowTitle('DAS_Visualizer')
    ...
    def initMenu(self):
        ...
        plot_3d_mag_psd_action = QAction('Plot 3D Magnitude PSD', plot_spectrum_menu)
        plot_3d_mag_psd_action.setStatusTip('Plot 3d magnitude psd')
    ...
    def initLayout(self):
        ...
        file_vbox.addWidget(file_path_label)
        file_vbox.addLayout(file_hbox)
        ...
        data_form_layout.setSpacing(20)
        data_form_layout.setHorizontalSpacing(10)
        data_form_layout.setLabelAlignment(Qt.AlignRight)  # 右对齐
        data_form_layout.addRow(Label('Channel'), self.channel_number_line_edit)
        data_form_layout.addRow(Label('Sampling Rate'), self.sampling_rate_line_edit)
        data_form_layout.addRow(Label('Sampling Times'), self.current_sampling_times_line_edit)
        data_form_layout.addRow(Label('Number of Channels'), self.current_channels_line_edit)

        # GPS时间组件
        gps_label1 = Label('GPS Time')
        gps_label1.setStyleSheet('font-size: 20px; font-family: "Times New Roman"')
        gps_label2 = Label('              From:                            To:')
        self.gps_year_from = LineEdit()
        self.gps_year_from.setFocusPolicy(Qt.NoFocus)
        self.gps_month_from = LineEdit()
        self.gps_month_from.setFocusPolicy(Qt.NoFocus)
        self.gps_day_from = LineEdit()
        self.gps_day_from.setFocusPolicy(Qt.NoFocus)
        self.gps_hour_from = LineEdit()
        self.gps_hour_from.setFocusPolicy(Qt.NoFocus)
        self.gps_minute_from = LineEdit()
        self.gps_minute_from.setFocusPolicy(Qt.NoFocus)
        self.gps_second_from = LineEdit()
        self.gps_second_from.setFocusPolicy(Qt.NoFocus)

        self.gps_year_to = LineEdit()
        self.gps_year_to.setFocusPolicy(Qt.NoFocus)
        self.gps_month_to = LineEdit()
        self.gps_month_to.setFocusPolicy(Qt.NoFocus)
        self.gps_day_to = LineEdit()
        self.gps_day_to.setFocusPolicy(Qt.NoFocus)
        self.gps_hour_to = LineEdit()
        self.gps_hour_to.setFocusPolicy(Qt.NoFocus)
        self.gps_minute_to = LineEdit()
        self.gps_minute_to.setFocusPolicy(Qt.NoFocus)
        self.gps_second_to = LineEdit()
        self.gps_second_to.setFocusPolicy(Qt.NoFocus)

        # GPS时间布局
        gps_form_layout_from = QFormLayout()
        gps_form_layout_from.setHorizontalSpacing(5)
        gps_form_layout_from.setLabelAlignment(Qt.AlignRight)
        gps_form_layout_from.addRow(Label('Year'), self.gps_year_from)
        gps_form_layout_from.addRow(Label('Month'), self.gps_month_from)
        gps_form_layout_from.addRow(Label('Day'), self.gps_day_from)
        gps_form_layout_from.addRow(Label('Hour'), self.gps_hour_from)
        gps_form_layout_from.addRow(Label('Minute'), self.gps_minute_from)
        gps_form_layout_from.addRow(Label('Second'), self.gps_second_from)

        gps_form_layout_to = QFormLayout()
        gps_form_layout_to.setHorizontalSpacing(5)
        gps_form_layout_to.setLabelAlignment(Qt.AlignRight)
        gps_form_layout_to.addRow(Label('Year'), self.gps_year_to)
        gps_form_layout_to.addRow(Label('Month'), self.gps_month_to)
        gps_form_layout_to.addRow(Label('Day'), self.gps_day_to)
        gps_form_layout_to.addRow(Label('Hour'), self.gps_hour_to)
        gps_form_layout_to.addRow(Label('Minute'), self.gps_minute_to)
        gps_form_layout_to.addRow(Label('Second'), self.gps_second_to)

        # 参数区整体布局
        params_vbox = QVBoxLayout()
        gps_hbox = QHBoxLayout()  # GPS时间布局
        gps_hbox.addLayout(gps_form_layout_from)
        gps_hbox.addLayout(gps_form_layout_to)
        params_vbox.addLayout(file_vbox)
        params_vbox.addWidget(self.files_table_widget)
        params_vbox.addStretch(1)
        params_vbox.addLayout(data_form_layout)
        params_vbox.addSpacing(50)
        params_vbox.addWidget(gps_label1)
        gps_label1.setAlignment(Qt.AlignCenter)
        params_vbox.addSpacing(10)
        params_vbox.addWidget(gps_label2)
        params_vbox.addSpacing(10)
        params_vbox.addLayout(gps_hbox)

        # 绘图区
        main_window_vbox.addWidget(self.tab_widget)

        # 主页面
        main_window_hbox.addLayout(params_vbox)
        main_window_hbox.addLayout(main_window_vbox)
        main_window_hbox.setStretchFactor(params_vbox, 1)
    ...
    def initParams(self):
        ...
        self.wavelet_dwt_decompose_level_isCal = False
    ...
    def plotSingleChannelTime(self):
        ...
        if self.data_unit_index == 0:
            ...
            self.plot_single_channel_time_widget.setLabel('left',
                                                          '<font face="Times New Roman">Phase Difference</font>')
        else:
            self.plot_single_channel_time_widget.setTitle(
    ...
    def selectDataFromTable(self, item):
        """当从文件列表中选择文件时更新图像等"""

        self.file_name = []
        self.file_name.append(item.text())
        self.readData()
        self.initParams()
        self.update()
    ...
    def updateDataGPSTime(self):
        ...
        # 更新开头文件GPS时间
        self.gps_year_from.setText(str(round(self.time[0][0])))
        self.gps_month_from.setText(str(round(self.time[0][1])))
        self.gps_day_from.setText(str(round(self.time[0][2])))
        self.gps_hour_from.setText(str(round(self.time[0][3])))
        self.gps_minute_from.setText(str(round(self.time[0][4])))
        self.gps_second_from.setText(str(round(self.time[0][5])))

        # 更新末尾文件GPS时间
        self.gps_year_to.setText(str(round(self.time[-1][0])))
        self.gps_month_to.setText(str(round(self.time[-1][1])))
        self.gps_day_to.setText(str(round(self.time[-1][2])))
        self.gps_hour_to.setText(str(round(self.time[-1][3])))
        self.gps_minute_to.setText(str(round(self.time[-1][4])))
        self.gps_second_to.setText(str(round(self.time[-1][5])))

    def updateTab(self):
        """更新tab名称"""

        self.tab_widget.setMovable(True)
        self.tab_widget.setTabText(2, f'Single - Window Method={self.window_text}')
    ...
    def exportData(self):
        ...
        if ftype.find('*.txt') > 0:
            ...
            data.to_excel(fpath, index=False, header=False)
    ...
    def plotFeature(self, text_list, features):
        ...
        pw = MyPlotWidget(feature_name, 'Channel', '')
        self.tab_widget.addTab(pw, f'{feature_name} - Window Method={self.window_text}')
        x = np.linspace(0, self.current_channels, self.current_channels)
        pw.setXRange(0, self.current_channels)
        pw.plot(x, feature_value, pen=QColor('blue'))
    ...
    def plotStrain(self):
        ...
        x = np.linspace(self.sampling_times_from_num, self.sampling_times_to_num,
                        self.current_sampling_times) / self.sampling_rate
        pw = MyPlotWidget('Strain Image', 'Time(s)', 'Strain(με)', grid=True)
        pw.setXRange(self.sampling_times_from_num / self.sampling_rate,
                     self.sampling_times_to_num / self.sampling_rate)
        pw.plot(x, data, pen=QColor('blue'))
        self.tab_widget.addTab(pw, f'Strain Image - Channel Number={self.channel_number}')
    ...
    def plotPSD(self):
        ...
        y = fixDateLength(self.current_sampling_times)
        data = 10.0 * np.log10(data ** 2 / self.current_sampling_times)[:y // 2]

        pw = MyPlotWidget('PSD', 'Frequency(Hz)', 'Power/Frequency(dB/Hz)', grid=True)
        x = np.arange(0, self.sampling_rate / 2, self.sampling_rate / self.current_sampling_times)
        pw.setXRange(10e-3, self.sampling_rate / 2)
        pw.setLogMode(x=True)
        pw.plot(x, data, pen=QColor('blue'))
        self.tab_widget.addTab(pw, f'PSD - Window Method={self.window_text}\n'
    ...
    def plot2dPSD(self):
        ...
        plt.pcolormesh(t, f, 10.0 * np.log10(Sxx), cmap='rainbow')
    ...
    def plot3dPSD(self):
        ...
        f, t, Sxx = spectrogram(data, self.sampling_rate, window=self.window_method(self.window_length, sym=False),
                                nperseg=self.window_length, noverlap=self.window_overlap_size, nfft=self.window_length,
                                scaling='density', mode='psd')
        im = ax.plot_surface(f[:, None], t[None, :], 10.0 * np.log10(Sxx), cmap='rainbow')
    ...
    def plotMagnitudeSpectrum(self):
        ...
        data = self.data[self.channel_number, :]
        y = fixDateLength(self.current_sampling_times)
        data = 10.0 * np.log10(np.abs(np.fft.fft(data)) / self.current_sampling_times)[:y // 2]

        pw = MyPlotWidget('Magnitude Spectrum', 'Frequency(Hz)', 'Magnitude(dB)', grid=True)
        x = np.arange(0, self.sampling_rate / 2, self.sampling_rate / self.current_sampling_times)
        pw.plot(x, data, pen=QColor('blue'))
        self.tab_widget.addTab(pw, f'Magnitude Spectrum - Window Method={self.window_text}\n'
                                   f'Channel Number={self.channel_number}')
    ...
    def plotAngleSpectrum(self):
        ...
        pw = MyPlotWidget('Angle Spectrum', 'Frequency(Hz)', 'Angle(rad)', grid=True)
        x = np.arange(0, self.sampling_rate / 2, self.sampling_rate / self.current_sampling_times)
        pw.plot(x, data, pen=QColor('blue'))
        self.tab_widget.addTab(pw, f'Angle Spectrum - Window Method={self.window_text}\n'
                                   f'Channel Number={self.channel_number}')
    ...
    def waveletDWTDialog(self):
        ...
        self.wavelet_dwt_reconstruct_le = LineEdit()
        self.wavelet_dwt_reconstruct_le.setFixedWidth(500)
        self.wavelet_dwt_reconstruct_le.setToolTip(
        ...
        self.wavelet_dwt_reconstruct_le.setText(str(self.wavelet_dwt_reconstruct))
        ...
        if not hasattr(self, 'wavelet_dwt_coeffs'):
            ...
            self.wavelet_dwt_reconstruct_le.setEnabled(False)
        ...
        self.wavelet_dwt_decompose_level_le = OnlyNumLineEdit()
        self.wavelet_dwt_decompose_level_le.setToolTip('Decomposition level, which must be an integer bigger than 0')
        self.wavelet_dwt_decompose_level_le.setText(str(self.wavelet_dwt_decompose_level))
        ...
        self.wavelet_dwt_decompose_level_checkbx.setToolTip(
            'If using calculated level or not, which is determined according to the data length and chosen wavelet')
        self.wavelet_dwt_decompose_level_checkbx.setChecked(self.wavelet_dwt_decompose_level_isCal)
        ...
        hbox2.addWidget(self.wavelet_dwt_reconstruct_le)

        hbox3.addWidget(wavelet_dwt_decompose_level_label)
        hbox3.addWidget(self.wavelet_dwt_decompose_level_le)
    ...
    def updateWaveletDWTParams(self):
        ...
        self.wavelet_dwt_reconstruct = self.wavelet_dwt_reconstruct_le.text()
        ...
        self.wavelet_dwt_decompose_level = int(self.wavelet_dwt_decompose_level_le.text())
        self.wavelet_dwt_decompose_level_isCal = self.wavelet_dwt_decompose_level_checkbx.isChecked()
    ...
    def runWaveletDWT(self):
        ...
        if self.wavelet_dwt_decompose_level_isCal:
    ...
    def waveletThresholdDialog(self):
        ...
        self.wavelet_threshold_le = NumPointLineEdit()
        self.wavelet_threshold_le.setToolTip('Thresholding value')
        self.wavelet_threshold_le.setText(str(self.wavelet_threshold))

        wavelet_threshold_sub_label = Label('Substitute value')
        self.wavelet_threshold_sub_le = NumPointLineEdit()
        self.wavelet_threshold_sub_le.setToolTip('Substitute value')
        self.wavelet_threshold_sub_le.setText(str(self.wavelet_threshold_sub))
        ...
        hbox.addWidget(self.wavelet_threshold_le)
        hbox.addWidget(wavelet_threshold_sub_label)
        hbox.addWidget(self.wavelet_threshold_sub_le)
        hbox.addStretch(1)
    ...
    def updateWaveletThresholdParams(self):
        ...
        self.wavelet_threshold = float(self.wavelet_threshold_le.text())
        self.wavelet_threshold_sub = float(self.wavelet_threshold_sub_le.text())
    ...
    def plotWaveletThreshold(self):
        ...
        data = self.data[self.channel_number, :]
        data = pywt.threshold(data, value=self.wavelet_threshold, mode=self.wavelet_threshold_mode_combx.currentText(),
    ...
    def waveletPacketsDialog(self):
        ...
        self.wavelet_packets_reconstruct_le = LineEdit()
        self.wavelet_packets_reconstruct_le.setFixedWidth(500)
        self.wavelet_packets_reconstruct_le.setToolTip(
        ...
        self.wavelet_packets_reconstruct_le.setText(str(self.wavelet_packets_reconstruct))
        ...
        if not hasattr(self, 'wavelet_packets_subnodes'):
            ...
            self.wavelet_packets_reconstruct_le.setEnabled(False)
        ...
        self.wavelet_packets_decompose_level_le = OnlyNumLineEdit()
        self.wavelet_packets_decompose_level_le.setToolTip(
            'Decomposition level, which must be an integer bigger than 0')
        self.wavelet_packets_decompose_level_le.setText(str(self.wavelet_packets_decompose_level))

        wavelet_packets_decompose_max_level_label = Label('Maximum Decompose Level')
        self.wavelet_packets_decompose_max_level_le = OnlyNumLineEdit()
        self.wavelet_packets_decompose_max_level_le.setFocusPolicy(Qt.NoFocus)
        self.wavelet_packets_decompose_max_level_le.setToolTip(
        ...
        self.wavelet_packets_decompose_max_level_le.setText(str(self.wavelet_packets_decompose_max_level))
        ...
        hbox2.addWidget(self.wavelet_packets_reconstruct_le)

        hbox3.addWidget(wavelet_packets_decompose_level_label)
        hbox3.addWidget(self.wavelet_packets_decompose_level_le)
        hbox3.addWidget(wavelet_packets_decompose_max_level_label)
        hbox3.addWidget(self.wavelet_packets_decompose_max_level_le)
        hbox3.addStretch(1)
    ...
    def updateWaveletPacketsParams(self):
        ...
        self.wavelet_packets_reconstruct = self.wavelet_packets_reconstruct_le.text()
        ...
        self.wavelet_packets_decompose_level = int(self.wavelet_packets_decompose_level_le.text())
        self.wavelet_packets_decompose_max_level = int(self.wavelet_packets_decompose_max_level_le.text())
    ...
    def runWaveletPackets(self):
        ...
        if self.wavelet_packets_flag:
            self.wavelet_packets_wp = pywt.WaveletPacket(data, wavelet=self.wavelet_packets_name_combx.currentText(),
